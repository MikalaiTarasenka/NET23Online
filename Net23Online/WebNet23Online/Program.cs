...
// === ВРЕМЕННАЯ ДИАГНОСТИКА ЛОКАЛИЗАЦИИ ===
...
Console.WriteLine("==================================================");
Console.WriteLine($"[DEBUG] Content Root Path: {env.ContentRootPath}");
Console.WriteLine($"[DEBUG] Supported Cultures: {string.Join(", ", locOptions.SupportedCultures.Select(c => c.Name))}");

var resPath = Path.Combine(env.ContentRootPath, "Localizations");
Console.WriteLine($"[DEBUG] Папка 'Localizations' существует: {Directory.Exists(resPath)}");

if (Directory.Exists(resPath))
{
    var files = Directory.GetFiles(resPath, "*.resx", SearchOption.AllDirectories);
    Console.WriteLine($"[DEBUG] Найдено .resx файлов: {files.Length}");
    foreach (var f in files.Take(5)) // Покажем первые 5, чтобы не засорять лог
    {
        Console.WriteLine($"  - {f}");
    }
}
...
{
    Console.WriteLine("[DEBUG] ОШИБКА: Папка Localizations не найдена в корне проекта!");
}
Console.WriteLine("==================================================");
// ==============================================
